...
class PreProcessor:
    ...
    def pre_process(gene_id_col_name=config.preprocessor_configs['gene_id_col_name'],
        ...
        :param gene_id_col_name: The column name of gene ID in `adata.var`.
        ...
        for file_path in Path(raw_data_directory).glob(f"*.{file_format}"):
            ...
            if gene_id_col_name not in adata.var.columns:
            ...
            adata.var = pd.merge(adata.var, gene_info_table, how='left', left_on=adata.var[gene_id_col_name],
